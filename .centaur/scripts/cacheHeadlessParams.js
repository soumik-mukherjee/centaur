...
module.exports.cacheHeadlessParams = function (args) {
    const [githubOrg, githubOrgRepo, runNumber, region, awsAccessKeyId, awsSecretAccessKey, envName] = args
    ...
    fs.writeFileSync(`./.centaur/cache/init-workflow/${runNumber}/amplify.json`, JSON.stringify(amplifyConfig));
    ...
    fs.writeFileSync(`./.centaur/cache/init-workflow/${runNumber}/awsCloudformationConfig.json`, JSON.stringify(awsCloudformationConfig));
    ...
    fs.writeFileSync(`./.centaur/cache/init-workflow/${runNumber}/reactConfig.json`, JSON.stringify(reactConfig));
    ...
    fs.writeFileSync(`./.centaur/cache/init-workflow/${runNumber}/frontend.json`, JSON.stringify(frontendConfig));

    // Setup providers 
    let providers = {awscloudformation: awsCloudformationConfig}
    fs.writeFileSync(`./.centaur/cache/init-workflow/${runNumber}/providers.json`, JSON.stringify(providers));
